...
File Name: QLab 5 - ETC EOS - Looping Control Cue Generator

This script was originally Taylor Glad's idea. Here's a link to his matching script:
https://www.taylorglad.com/post/create-cues-looping-and-range-qlab-5
...
set qlabCueType to "Network" -- One of: {"Network", "MIDI"}. Use "" to prompt.
set qlabCuePatch to 1 -- The patch number, for either Network or MIDI. Use 0 to prompt.
set qlabMidiDeviceID to 0 -- This doesn't matter if you are making a network cue. Use -1 to prompt.
...
"
		...
		set cueDialog to result
		if button returned of cueDialog is "Cancel (All Done)" then return
		set currentCueNumber to text returned of cueDialog as text
